...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const inputClass = "w-full bg-transparent border-2 border-white/10 rounded-xl p-5 pl-14 focus:border-[#CCFF00] outline-none font-display font-bold uppercase text-xs tracking-widest text-white";

  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          <motion.div {...fadeProps} className="absolute inset-0 bg-black/90 backdrop-blur-xl" onClick={onClose} />
            ...
            <div className="p-10">
                ...
                <div className="space-y-8">
                  {regStep === 1 ? (
                    <div className="space-y-8 text-center">
                      <div className="w-16 h-16 border-2 border-white/20 rounded-xl flex items-center justify-center mx-auto opacity-70"><Key size={24} /></div>
                      <p className="text-[10px] font-body font-black tracking-widest text-white/70 uppercase">ENTER THE REGISTRATION KEY PROVIDED BY YOUR ARCHITECT</p>
                      <input value={keyInput} onChange={e => setKeyInput(e.target.value.toUpperCase())} placeholder="SAL-XXXX-XXXX" className="w-full bg-transparent border-2 border-white/10 rounded-xl p-5 text-center focus:border-[#CCFF00] outline-none font-display font-black uppercase text-xl tracking-[0.2em] text-white" />
                    ...
                    <form onSubmit={handleFinalizeReg} className="space-y-6">
                      <div className="flex items-center gap-4 p-4 glass rounded-xl mb-4">
                        ...
                        <span className="text-[9px] font-body font-black tracking-widest text-white/70 uppercase">KEY VALIDATED. SETUP CREDENTIALS.</span>
                      </div>
                      <div className="relative">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-white/70" size={18} />
              ...
              <div className="text-center pt-10">
                <button type="button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setRegStep(1); setError(''); }}
                  className="text-[10px] font-body font-black tracking-widest uppercase transition-colors text-white/70 hover:text-white">
