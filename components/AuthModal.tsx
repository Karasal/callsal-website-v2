...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const inputClass = "w-full bg-transparent border-2 border-gray-200 rounded-xl p-5 pl-14 focus:border-[#CCFF00] outline-none font-display font-bold uppercase text-xs tracking-widest text-gray-900";
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <motion.div {...modalProps} className="relative w-full max-w-xl glass-strong rounded-2xl overflow-hidden">
            <div className="p-8 border-b border-gray-200 flex items-center justify-between bg-gray-50">
              <h2 className="text-3xl font-display font-black uppercase tracking-tighter text-gray-900">
                {mode === 'login' ? 'ACCESS HUB' : 'PRIVATE VAULT'}
              </h2>
              <button onClick={onClose} className="p-2 rounded-lg transition-colors hover:bg-gray-100 text-gray-900"><X size={20} /></button>
            </div>
            ...
            <div className="p-10">
                ...
                <div className="space-y-8">
                  {regStep === 1 ? (
                    <div className="space-y-8 text-center">
                      <div className="w-16 h-16 border-2 border-gray-200 rounded-xl flex items-center justify-center mx-auto text-gray-400"><Key size={24} /></div>
                      <p className="text-[10px] font-body font-black tracking-widest text-gray-500 uppercase">ENTER THE REGISTRATION KEY PROVIDED BY YOUR ARCHITECT</p>
                      <input value={keyInput} onChange={e => setKeyInput(e.target.value.toUpperCase())} placeholder="SAL-XXXX-XXXX" className="w-full bg-transparent border-2 border-gray-200 rounded-xl p-5 text-center focus:border-[#CCFF00] outline-none font-display font-black uppercase text-xl tracking-[0.2em] text-gray-900" />
                    ...
                    <form onSubmit={handleFinalizeReg} className="space-y-6">
                      <div className="flex items-center gap-4 p-4 glass rounded-xl mb-4">
                        ...
                        <span className="text-[9px] font-body font-black tracking-widest text-gray-500 uppercase">KEY VALIDATED. SETUP CREDENTIALS.</span>
                      </div>
                      <div className="relative">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={18} />
              ...
              <div className="text-center pt-10">
                <button type="button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setRegStep(1); setError(''); }}
                  className="text-[10px] font-body font-black tracking-widest uppercase transition-colors text-gray-500 hover:text-gray-900">
